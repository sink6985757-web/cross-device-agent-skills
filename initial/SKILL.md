...
2. 在 runtime 偵測 OS、Git、`gh auth status`、共用技能目錄、已安裝 Agent 與可用 Obsidian vault。不得讀取或保存 token、`.env`、credential cache、裝置名稱或絕對路徑。
...
5. 依 `TEMPLATE.md` 建立缺少的 `AGENTS.md` 與 `handoff.md`；專案名稱、目標與時程沒有證據時留待確認，不得猜測。
